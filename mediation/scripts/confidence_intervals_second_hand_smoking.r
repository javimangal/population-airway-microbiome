...
calculate_total_effect <- function(data_temp) {
  model_total <- glm(
    Airway_disease ~ SHS_binary, 
    weights = data_temp$weight1, 
    data = data_temp, 
    family = binomial(link = logit)
  )
  
  total_effect_exposure <- coef(model_total)["SHS_binary2"]
...
calculate_direct_effect <- function(data_temp) {
  model_both <- glm(
    Airway_disease ~ SHS_binary + AMHI, 
    weights = data_temp$weight, 
    data = data_temp, 
    family = binomial(link = logit)
  )
  
  direct_effect_exposure <- coef(model_both)["SHS_binary2"]
