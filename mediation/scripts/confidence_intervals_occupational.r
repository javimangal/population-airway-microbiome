...
calculate_total_effect <- function(data_temp) {
  model_total <- glm(
    Airway_disease ~ Occupational_pollution, 
    weights = data_temp$weight1, 
    data = data_temp, 
    family = binomial(link = logit)
  )
  
  total_effect_exposure <- coef(model_total)["Occupational_pollution2"]
...
calculate_direct_effect <- function(data_temp) {
  model_both <- glm(
    Airway_disease ~ Occupational_pollution + AMHI, 
    weights = data_temp$weight, 
    data = data_temp, 
    family = binomial(link = logit)
  )
  
  direct_effect_exposure <- coef(model_both)["Occupational_pollution2"]
